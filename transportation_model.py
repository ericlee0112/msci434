...
# Creates a 2d array of costs of each transportation path
costs = [   #Bars
         #1 2 3 4 5
...
model = LpProblem("Beer Distribution Problem",LpMinimize)
...
# The supply maximum constraints are added to model for each supply node (warehouse)
for w in Warehouses:
    model += lpSum([vars[w][b] for b in Bars])<=supply[w], "Sum_of_Products_out_of_Warehouse_%s"%w

# The demand minimum constraints are added to model for each demand node (bar)
for b in Bars:
